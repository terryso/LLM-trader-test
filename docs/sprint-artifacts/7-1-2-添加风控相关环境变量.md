...
- [ ] **Task 1**: 在 `config/settings.py` 中加载风控相关环境变量（AC: 1）
  - [ ] 1.1 定义 `RISK_CONTROL_ENABLED` / `KILL_SWITCH` / `DAILY_LOSS_LIMIT_ENABLED` / `DAILY_LOSS_LIMIT_PCT`
  - [ ] 1.2 使用统一的布尔解析策略（如 `"true" / "1" / "yes"` 视为真）
  - [ ] 1.3 将 `DAILY_LOSS_LIMIT_PCT` 解析为 `float`，设置默认值 `5.0`

- [ ] **Task 2**: 在 `.env.example` 中更新风控相关配置示例（AC: 2）
  - [ ] 2.1 添加 4 个新环境变量条目
  - [ ] 2.2 为每个变量补充用途说明和默认行为
  - [ ] 2.3 标注「开启实盘 / 提高阈值需自担风险」等警示语

- [ ] **Task 3**: 添加配置验证逻辑（AC: 3）
  - [ ] 3.1 对 `DAILY_LOSS_LIMIT_PCT` 做范围检查（例如 `0 < 值 <= 100`）
  - [ ] 3.2 无效值时记录 warning 日志，并回退到默认值
  - [ ] 3.3 如有需要，为未来扩展其他风控阈值预留验证入口

- [ ] **Task 4**: 添加单元测试（AC: 4）
  - [ ] 4.1 在现有 `tests/test_config_and_env.py` 或新建专门测试文件中覆盖风控配置
  - [ ] 4.2 测试默认配置：在未设置环境变量时得到预期默认值
  - [ ] 4.3 测试环境变量覆盖：设置不同组合的布尔值 / 百分比
  - [ ] 4.4 测试无效数值输入时回退到默认值并记录日志（可使用 caplog）
...
- 本 Story 当前处于 `drafted` 状态，尚未开始实现与测试。完成后将在此处补充：
  - ✅ 配置加载逻辑实现情况
  - ✅ `.env.example` 更新情况
  - ✅ 单元测试覆盖情况与测试结果
...
**NEW (expected):**
- 无新增 Python 模块文件，仅更新现有配置与测试文件。

**MODIFIED (expected):**
- `config/settings.py` - 新增风控相关配置常量与解析逻辑
- `.env.example` - 新增风控相关环境变量示例
- `tests/test_config_and_env.py`（或等价文件）- 新增风控配置单元测试
